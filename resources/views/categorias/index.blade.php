  ...
  <div class="content-limiter">
    ...
    <div class="page-head">
      <div class="page-head__left">
        ...
        <div>
          <h1 class="page-head__title">Categorias de Produtos</h1>
          <p class="page-head__subtitle">Liste, pesquise e gerencie as categorias de produtos.</p>
        </div>
      </div>
      ...
      <button type="button"
              ...
              data-bs-content="Busque pela descrição. Clique no nome para visualizar. Use as ações à direita para ver, editar ou excluir."
              aria-label="Ajuda sobre a lista de categorias">
        <i class="fa-regular fa-circle-question me-2"></i> Ajuda
      </button>
    </div>

    {{-- Balão fixo --}}
    <div class="hint-bubble" id="hint-lista-categorias" role="status" aria-live="polite" style="display:block">
      <div class="d-flex align-items-start gap-2">
        <i class="fa-regular fa-circle-question hint-bubble__icon mt-1"></i>
        <div class="flex-grow-1">
          <strong>Dica rápida</strong><br>
          Pesquise pela <em>Descrição</em>. Clique na descrição para <strong>visualizar</strong>.
    ...
    <div class="mb-3">
      <a href="{{ route('categorias.create') }}" class="btn btn-primary">
        <i class="fa-solid fa-plus me-2"></i> Cadastrar Categoria
      </a>
    </div>
      ...
      <div class="data-card data-card--empty">
        <div class="empty-state">
          ...
          <div class="empty-state__text">
            ...
            <a href="{{ route('categorias.create') }}">Cadastrar agora</a>.
          </div>
        </div>
      </div>
    @else
      @php
        $perPage = (int) request('per_page', $categorias->perPage());
        $sizes = [10,25,50,100];
      @endphp
        ...
        <div class="table-responsive">
          <table class="table table-hiper align-middle">
            <colgroup>
              <col>                      {{-- Descrição --}}
              <col>                      {{-- Observação --}}
              <col style="width:160px">  {{-- Ações (3 botões) --}}
            </colgroup>
            ...
            @foreach($categorias as $c)
              <tr>
                <td class="cell-name">
                  <a href="{{ route('categorias.show', $c->id) }}" class="name-link" title="Ver categoria">
                    {{ $c->descricao }}
                  </a>
                </td>

                <td class="nowrap text-truncate" style="max-width:480px">{{ $c->observacao ?? '-' }}</td>

                <td class="text-end">
                  <div class="btn-group btn-group-actions">
                    <a class="btn btn-icon" href="{{ route('categorias.show', $c->id) }}" title="Ver">
                      <i class="fa-regular fa-eye"></i>
                    </a>
                    <a class="btn btn-icon" href="{{ route('categorias.edit', $c->id) }}" title="Editar">
                      <i class="fa-regular fa-pen-to-square"></i>
                    </a>

                    {{-- Form de exclusão + botão que abre o modal bonito --}}
                    <form id="del-cat-{{ $c->id }}" action="{{ route('categorias.destroy', $c->id) }}" method="POST" class="d-inline">
                      @csrf @method('DELETE')
                      <button type="button"
                              class="btn btn-icon text-danger js-open-delete"
                              data-form="del-cat-{{ $c->id }}"
                              data-title="Remover categoria?"
                              data-message="Remover <strong>{{ $c->descricao }}</strong>? Essa ação não pode ser desfeita."
                              title="Excluir">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content modal-confirm">
      ...
      <div class="modal-body pt-0">
        <h5 class="modal-title fw-bold mb-1" id="confirmDeleteTitle">Remover categoria?</h5>
